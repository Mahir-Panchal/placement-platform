...
def user(db):
    return User.objects.create_user(
        email="resume@test.com", password="Test@1234", full_name="Resume Tester"
    )
...
def auth_client(user):
    ...
    response = client.post(
        reverse("login"),
        {"email": "resume@test.com", "password": "Test@1234"},
        format="json",
    )
...
def test_upload_non_pdf_rejected(auth_client):
    ...
    fake_file = SimpleUploadedFile(
        "resume.txt", b"This is not a PDF", content_type="text/plain"
    )
    url = reverse("resume_upload")
...
def test_resume_status_endpoint(auth_client, sample_pdf):
    ...
    upload_response = client.post(
        reverse("resume_upload"), {"file": sample_pdf}, format="multipart"
    )
