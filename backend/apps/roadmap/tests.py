...
def user(db):
    return User.objects.create_user(
        email="roadmap@test.com", password="Test@1234", full_name="Roadmap Tester"
    )
...
def auth_client(user):
    ...
    response = client.post(
        reverse("login"),
        {"email": "roadmap@test.com", "password": "Test@1234"},
        format="json",
    )
...
def test_generate_roadmap(auth_client):
    ...
    response = client.post(
        url, {"target_role": "SDE-1 at product startup"}, format="json"
    )
...
def test_generate_roadmap_unauthenticated(client):
    ...
    url = reverse("roadmap_generate")
    response = client.post(url, {"target_role": "SDE-1"}, format="json")
...
def test_roadmap_status(auth_client):
    ...
    roadmap = Roadmap.objects.create(
        user=user, target_role="SDE-1", status="DONE", content={"weeks": []}
    )

    response = client.get(reverse("roadmap_status", kwargs={"pk": roadmap.id}))
    assert response.status_code == 200
